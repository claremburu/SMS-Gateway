...
export const register = (vals) => (dispatch) => {
  ...
  axios
    .post(`${baseUrl}/v1/onfonapps/clients`, body, config)
    .then((res) => {
      const { user } = res.data.data;
...
export const requestCode = (email) => (dispatch) => {
  ...
  axios
    .post(`${baseUrl}/v1/onfonapps/clients:requestChangePassword`, body)
    .then((res) => {
      const user = res.data;
...
export const createPassword = (password) => (dispatch) => {
  const body = {
    password
  };
  ...
  axios
    .put(`${baseUrl}/v1/onfonapps/clients:updatePassword`, body)
    .then((res) => {
      const user = res.data;
...
export const updateCredentials = (vals) => (dispatch, getState) => {
  ...
  axios
    .put(`${baseUrl}/v1/onfonapps/clients:updateClientField`, body, authConfig(getState().auth?.token))
    .then((res) => {
      const info = res.data;
...
export const logout = () => (dispatch, getState) => {
  ...
  axios
    .delete(`${baseUrl}/v1/onfonapps/clients/{clientId}`, config)
    .then(() => {
